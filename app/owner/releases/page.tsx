...
export default function OwnerReleasesPage() {
  ...
  async function login(event: FormEvent) {
    ...
    const response = await fetch('/api/owner/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ firstSecret, secondSecret }) })
    if (response.ok) { setAuthed(true); setFirstSecret(''); setSecondSecret(''); await load() } else setMessage((await response.json()).error || 'Access denied.')
    setBusy(false)
  }
